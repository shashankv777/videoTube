...
const generateAccessandRefereshTokens = async(userId)=>{
  try{
    ...
    throw new ApiError(500,"Somenthing went wrong while generating referesh and access token")
  }
}

const registerUser = asyncHandler(async (req,res)=>{
    // get user details from frontend
    // validation of the details (it should not be empty etc.)
...
const loginUser = asyncHandler(async(req,res)=>{
 ...
 const {accessToken,refreshToken} = await generateAccessandRefereshTokens(user._id)
  ...
  const options ={
    httpOnly:true,    //modification of cookies only from derver side allowed
    secure:false,
  }
